...
interface ConsensusResult {
  finalResult: Record<string, unknown> | string;
  consensus: number;
...
- Clear about confidence levels`;
   ...
   */
  private buildConsensus(results: AnalysisResult[]): ConsensusResult {
  ...
  private calculateConsensus(results: AnalysisResult[]): { result: Record<string, unknown> | string; score: number } {
  ...
  private mergeJsonResponses(responses: Record<string, unknown>[]): Record<string, unknown> {
  ...
  private selectBestValue(values: unknown[]): unknown {
   ...
   */
  protected validateFactBased(analysis: Record<string, unknown>, providedData: Record<string, unknown>): boolean {
...
export abstract class MizanAgent extends BaseAgent {
  ...
  private assessDataQuality(data: Record<string, unknown>): {
    completeness: number;
    totalFields: number;
    filledFields: number;
  ...
  private countFields(obj: Record<string, unknown>): number {
    ...
    for (const key in obj) {
      if (typeof obj[key] === 'object' && obj[key] !== null && !Array.isArray(obj[key])) {
        count += this.countFields(obj[key] as Record<string, unknown>);
      } else {
        count++;
      }
    }
  ...
  private countFilledFields(obj: Record<string, unknown>): number {
    let count = 0;
    for (const key in obj) {
      if (typeof obj[key] === 'object' && obj[key] !== null && !Array.isArray(obj[key])) {
        count += this.countFilledFields(obj[key] as Record<string, unknown>);
  ...
  private identifyMissingCriticalData(data: Record<string, unknown>): string[] {
