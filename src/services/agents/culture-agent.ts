...
import { RecognitionAgent, RecognitionInput, RecognitionAnalysis } from './recognition/recognition-agent.js';
import { EngagementAgent, EngagementInput, EngagementAnalysis } from './engagement/engagement-agent.js';
...
export interface CultureAnalysisOutput {
  ...
  engagementAnalysis?: EngagementAnalysis; // Results from Engagement Agent
  recognitionAnalysis?: RecognitionAnalysis; // Results from Recognition Agent
...
export class CultureAgent extends ThreeEngineAgent {
  ...
  async analyzeCompanyCulture(input: CultureAnalysisInput): Promise<CultureAnalysisOutput> {
    ...
    // Fetch assessment data to get engagement and recognition scores
    const assessments = await this.fetchAssessments(input);

    // Initialize agent analyses
    let engagementAnalysis: EngagementAnalysis | undefined;
    let recognitionAnalysis: RecognitionAnalysis | undefined;

    // If we have assessments with scores, trigger sub-agents
    if (assessments && assessments.length > 0) {
      try {
        // Trigger Engagement Agent (Question 4)
        const engagementInputs: EngagementInput[] = assessments
          .filter(a => a.engagement !== null && a.engagement !== undefined)
          .map(a => ({
            score: a.engagement as number,
            personalValues: Array.isArray(a.personalValues) ? a.personalValues : [],
            currentExperience: Array.isArray(a.currentExperience) ? a.currentExperience : [],
            desiredExperience: Array.isArray(a.desiredExperience) ? a.desiredExperience : [],
            tenantId: input.tenantId,
            userId: a.userId
          }));

        if (engagementInputs.length > 0) {
          const engagementAgent = new EngagementAgent('engagement', this.config);
          
          // Analyze aggregated engagement
          const aggregatedEngagementAnalysis = await engagementAgent.analyzeAggregatedEngagement(engagementInputs);
          engagementAnalysis = {
            score: aggregatedEngagementAnalysis.averageScore,
            interpretation: aggregatedEngagementAnalysis.interpretation,
            factors: aggregatedEngagementAnalysis.commonFactors,
            drivers: aggregatedEngagementAnalysis.commonDrivers,
            barriers: aggregatedEngagementAnalysis.organizationalBarriers,
            correlations: {
              withPersonalValues: 'Aggregated engagement correlations analyzed',
              withCultureGaps: 'Engagement-culture gap correlations identified',
              withDesiredExperience: 'Engagement-aspiration correlations found'
            },
            recommendations: aggregatedEngagementAnalysis.recommendations,
            riskFactors: aggregatedEngagementAnalysis.averageScore < 3 
              ? ['Low organizational engagement detected', 'Risk of widespread disengagement']
              : [],
            confidence: 0.85,
            metadata: {
          ...
          console.log(`✅ Engagement Agent analyzed ${engagementInputs.length} employees`);
        }
      } catch (error) {
      ...
      try {
        // Trigger Recognition Agent (Question 5)
        const recognitionInputs: RecognitionInput[] = assessments
          .filter(a => a.recognition !== null && a.recognition !== undefined)
          .map(a => ({
            score: a.recognition as number,
            personalValues: Array.isArray(a.personalValues) ? a.personalValues : [],
            currentExperience: Array.isArray(a.currentExperience) ? a.currentExperience : [],
            desiredExperience: Array.isArray(a.desiredExperience) ? a.desiredExperience : [],
            tenantId: input.tenantId,
        ...
        if (recognitionInputs.length > 0) {
          const recognitionAgent = new RecognitionAgent('recognition', this.config);
          
          // Analyze aggregated recognition
          const aggregatedRecognitionAnalysis = await recognitionAgent.analyzeAggregatedRecognition(recognitionInputs);
          recognitionAnalysis = {
            score: aggregatedRecognitionAnalysis.averageScore,
            interpretation: aggregatedRecognitionAnalysis.interpretation,
            patterns: aggregatedRecognitionAnalysis.commonPatterns,
            needs: aggregatedRecognitionAnalysis.organizationalNeeds,
            correlations: {
              withPersonalValues: 'Aggregated recognition correlations analyzed',
              withCultureGaps: 'Recognition-culture gap correlations identified',
              withDesiredExperience: 'Recognition-aspiration correlations found'
            },
            recommendations: aggregatedRecognitionAnalysis.recommendations,
            riskFactors: aggregatedRecognitionAnalysis.averageScore < 3
              ? ['Low organizational recognition detected', 'Risk of employee feeling undervalued']
              : [],
            confidence: 0.85,
            metadata: {
              analysisDate: new Date(),
          ...
          console.log(`✅ Recognition Agent analyzed ${recognitionInputs.length} employees`);
        }
      } catch (error) {
        console.error('Recognition Agent analysis failed:', error);
        recognitionAnalysis = undefined;
      }
    }

    // Integrate agent analyses into culture output
  ...
  /**
   * Fetch assessments for analysis
   */
  private async fetchAssessments(input: CultureAnalysisInput): Promise<any[]> {
    try {
      if (input.assessmentIds && input.assessmentIds.length > 0) {
        // Fetch specific assessments
        const assessments = await db.select()
          .from(cultureAssessments)
          .where(eq(cultureAssessments.tenantId, input.tenantId));
        
        return assessments.filter(a => input.assessmentIds!.includes(a.id));
      } else if (input.targetType === 'company') {
        // Fetch all assessments for company
        return await db.select()
          .from(cultureAssessments)
          .where(eq(cultureAssessments.tenantId, input.tenantId));
      } else if (input.targetType === 'individual' && input.targetId) {
        // Fetch individual assessment
        return await db.select()
          .from(cultureAssessments)
          .where(eq(cultureAssessments.userId, input.targetId));
      }
