...
class AIProviderRouter {
    constructor() {
        this.openai = new openai_1.OpenAI({
            ...
            timeout: timeout_1.AI_REQUEST_TIMEOUT
        });
        this.anthropic = new sdk_1.default({
            apiKey: process.env.ANTHROPIC_API_KEY,
            timeout: timeout_1.AI_REQUEST_TIMEOUT
        });
        this.gemini = new generative_ai_1.GoogleGenerativeAI(process.env.GOOGLE_API_KEY);
        this.mistral = new mistralai_1.default(process.env.MISTRAL_API_KEY);
    }
    async invokeProvider(provider, call) {
    ...
    async invokeEnsemble(call) {
        const providers = ['openai', 'anthropic', 'gemini', 'mistral'];
        const results = await Promise.allSettled(providers.map(provider => this.invokeProvider(provider, call)));
        ...
        const bestResult = successfulResults.reduce((best, current) => current.confidence > best.confidence ? current : best);
        return {
            ...bestResult,
            provider: 'openai', // Use first provider as identifier for ensemble
            confidence: successfulResults.reduce((sum, r) => sum + r.confidence, 0) / successfulResults.length
        };
    }
    async invokeOpenAI(call) {
        try {
    ...
    async invokeGemini(call) {
        try {
            ...
            const result = await (0, timeout_1.withTimeout)(generatePromise, timeout_1.AI_REQUEST_TIMEOUT, 'Gemini');
    ...
    async invokeMistral(call) {
        try {
            ...
            // ✅ PRODUCTION: Unified timeout with automatic cleanup
            const response = await (0, timeout_1.withTimeout)(chatPromise, timeout_1.AI_REQUEST_TIMEOUT, 'Mistral');
            const content = response.choices[0].message.content;
            // ✅ Validate content exists before creating response (fail-fast, not defensive)
            if (!content || content.trim().length === 0) {
...
exports.aiRouter = new AIProviderRouter();
// Export convenience functions
async function invokeProvider(provider, call) {
    return exports.aiRouter.invokeProvider(provider, call);
}
