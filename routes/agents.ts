...
const analysisRequestSchema = z.object({
  agentType: z.enum(['culture', 'structure', 'skills', 'performance', 'engagement', 'recognition', 'benchmarking']),
  inputData: z.any(),
...
router.post('/skills/analyze', authenticateToken, authorize(['admin', 'manager']), async (req, res) => {
  try {
    ...
    const { targetType = 'company', targetId, strategyId } = req.body;
...
router.post('/benchmarking/compare', authenticateToken, authorize(['admin']), async (req, res) => {
  try {
    ...
    const { metrics, industry, period } = req.body;
